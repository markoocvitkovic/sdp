...
for ime_datoteke in os.listdir(putanja_do_direktorija):
    if ime_datoteke.endswith('.arff'):
        ...
        for k, (točnost, f1) in sfs_rezultati.items():
            print(f'Točnost s {k} značajki: {točnost:.4f}, F1-mjera: {f1:.4f}')

        korelacije = pearson_korelacija(X_train, y_train)
        sortirani_indeksi = np.argsort(korelacije)[::-1]  # Sortiraj značajke po korelaciji
        ...
        for k, (točnost, f1) in mi_rezultati.items():
            print(f'Točnost s {k} značajki: {točnost:.4f}, F1-mjera: {f1:.4f}')
        ...
        for k, (točnost, f1) in anova_rezultati_k.items():
            print(f'Točnost s {k} značajki: {točnost:.4f}, F1-mjera: {f1:.4f}')
